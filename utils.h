...
#define WORK_DIR "/scratch0/tenvisdb/"

#define INVALID_SLOT (0xFFFFFFFF)

#define NUM_SEGMENTS (2048LU)
...
#define periodic_count_id(period, id, msg, args...) \
   do { \
      rdtscll(__last); \
      __nb_count++; \
      if(cycles_to_us(__last - __start) > ((period)*1000LU)) { \
         printf("thread %lu (%s,%d) [ %lu s] [ %lu ops/s] " msg "\n", id,  __FUNCTION__ , __LINE__, cycles_to_us(__last - __real_start)/1000000LU, __nb_count*1000000LU/cycles_to_us(__last - __start), ##args); \
         __nb_count = 0; \
         __start = __last; \
      } \
   } while(0);
